...
data <- readGEORawFile("gse52865luma_norm.txt", sep = "\t", Uname = "unmeth", Mname = "\\.meth", row.names = 1, array = "IlluminaHumanMethylation450k")

grs <- ratioConvert(data) # Calculamos el estadístico beta con el que se computan las dmrs

g <- as.factor(c(rep("c", 17), rep("luma", 12))) # Especificamos los controles y los casos
...
save(dmrs, file = "dmrs52865luma.RData")
