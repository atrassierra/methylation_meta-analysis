...
data <- readGEORawFile("tableilb_norm.txt", sep = "\t", Uname = "Signal_A", Mname = "Signal_B", row.names = 1, array = "IlluminaHumanMethylation450k")

grs <- ratioConvert(data) # Calculamos el estadístico beta con el que se computan las dmrs

g <- as.factor(c(rep("c", 4), rep("ilb", 10))) # Especificamos los controles y los casos
...
save(dmrs, file = "dmrs59901ilb.RData")
