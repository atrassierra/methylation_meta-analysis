#########################################################################################
##                                                                                     ##
## s06_functional_meta_analysis_kegg.r                                                 ##
...
for (i in methods){
  ...
  if (file.exists(file.path (.job$dir, "meta", "kegg", "files"))){
    setwd(file.path(.job$dir, "meta", "kegg", "files"))
  } else {
    dir.create(file.path(.job$dir, "meta", "kegg", "files"), recursive = T)
    setwd(file.path(.job$dir, "meta", "kegg", "files"))    
  }
...
if (file.exists(file.path (.job$dir, "meta", "kegg", "plots"))){
  setwd(file.path(.job$dir, "meta", "kegg", "plots"))
} else {
  dir.create(file.path(.job$dir, "meta", "kegg", "plots"), recursive = T)
  setwd(file.path(.job$dir, "meta", "kegg", "plots"))    
}
